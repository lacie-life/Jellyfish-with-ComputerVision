...
int main(int argc, char** argv) {
    ...
    if (pcl::console::find_argument(argc, argv, "-m") >= 0) {
        ...
        std::cout << "Setting unseen values in range image to maximum range reading. \n";
    }
    ...
    if (!pcd_filename_indices.empty()) {
        ...
        std::string far_ranges_filename = pcl::getFilenameWithoutExtension(filename) + "_far_range.pcd";
        if (pcl::io::loadPCDFile(far_ranges_filename.c_str(), far_ranges) == -1) {
            std::cout << "Far ranges file \"" << far_ranges_filename << "\" dose not exist. \n";
        } else {
            setUnseenToMaxRange = true;
    ...
    // Creat RangeImage from the PointCloud
    ...
    // Show keypoints in ranges image widget
